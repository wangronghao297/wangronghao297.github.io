window.SITE_DATA = {
  name: "我的个人网站",
  email: "hello@example.com",
  hero: {
    kicker: "个人记录",
    title: "把文章、照片和生活片段放在同一个地方。",
    subtitle: "这里会慢慢长出你的想法、旅途、作品和日常。",
    image:
      "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=1800&q=85",
  },
  intro:
    "一些观察、想法、学习记录，以及值得留下来的瞬间。",
  about:
    "你好，我正在搭建自己的个人网站。这里会放文章、照片、项目和一些认真生活的证据。",
  profile: [
    ["位置", "中国"],
    ["主题", "文章、摄影、生活记录"],
    ["邮箱", "hello@example.com"],
  ],
  articles: [
    {
      title: "第一篇文章：从这里开始",
      date: "2026-05-25",
      excerpt:
        "这是网站上的第一篇文章。你可以把它换成自己的故事、想法或学习记录。",
      cover:
        "https://images.unsplash.com/photo-1499750310107-5fef28a66643?auto=format&fit=crop&w=1200&q=85",
      body: [
        "这是我的个人网站的第一篇文章。",
        "以后我会在这里写下最近的思考、读到的书、做过的项目、旅行中的片段，或者任何我想长期保存的内容。",
        "要更新文章，只需要打开 site-data.js，在 articles 数组里增加一项。"
      ],
    },
    {
      title: "一些值得保存的瞬间",
      date: "2026-05-24",
      excerpt:
        "照片和文字放在一起，会让很多普通的日子变得更容易被记住。",
      cover:
        "https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=1200&q=85",
      body: [
        "照片不一定要很正式。它可以是一次散步、一顿饭、一束光，或者一个安静的下午。",
        "这个网站的照片区可以放作品，也可以放生活。",
        "等你换成自己的照片后，它就会更像你。"
      ],
    },
    {
      title: "给未来的自己留一个入口",
      date: "2026-05-23",
      excerpt:
        "个人网站像一个慢慢生长的房间，不需要一开始就完整，但值得先拥有。",
      cover:
        "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=1200&q=85",
      body: [
        "很多内容一开始都只是零散的笔记。",
        "当它们被整理到同一个地方，时间久了就会变成一条能回头看的线索。",
        "这里就是那条线索的起点。"
      ],
    },
  ],
  photos: [
    {
      title: "山间傍晚",
      location: "旅途",
      src:
        "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=900&q=85",
    },
    {
      title: "城市一角",
      location: "日常",
      src:
        "https://images.unsplash.com/photo-1494526585095-c41746248156?auto=format&fit=crop&w=900&q=85",
    },
    {
      title: "安静工作台",
      location: "记录",
      src:
        "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=900&q=85",
    },
    {
      title: "夜色",
      location: "散步",
      src:
        "https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=900&q=85",
    },
  ],
};
